...
class PatternTest:
    ...
    def _calc(self, img_base, img_test, method):
        if method == CROSS_CORRELATION:
            ...
            result = np.correlate(self._normalize_img(
                img_base), self._normalize_img(img_test))[0]
...
def read_img_to_list(fig_dir):
    fig_list = os.listdir(fig_dir)
    fig_list.sort()
    figs = list(map(lambda fn: imageio.imread(
        os.path.join(fig_dir, fn)), fig_list))
    return figs


def make_gif_from_img_list(img_list, result_path, format='GIF', duration=0.5):
    imageio.mimsave(os.path.join(result_path, 'output.gif'),
                    img_list, format=format, duration=duration)


def make_gif(fig_dir, result_path):
    fig_list = os.listdir(fig_dir)
    fig_list.sort()
    # print(fig_list)
    figs = list(map(lambda fn: imageio.imread(
        os.path.join(fig_dir, fn)), fig_list))
    imageio.mimsave(os.path.join(result_path, 'output.gif'),
                    figs, format='GIF', duration=0.7)


if __name__ == "__main__":
    # test_write_to_zarr()
    figs_dir1 = '/n/groups/htem/Segmentation/xg76/local_realignment/result/desc'
    figs_dir2 = '/n/groups/htem/Segmentation/xg76/local_realignment/result/orig'
    res_dir = '/n/groups/htem/Segmentation/xg76/local_realignment/result'
    # make_gif(figs_dir, res_dir)
    figs = read_img_to_list(figs_dir2)
    figs.extend(read_img_to_list(figs_dir1))
    make_gif_from_img_list(figs, res_dir)